...
// it is used as a lookup table to make finding distinct colors in a gradient a lookup
...
// generateColor will for a given index (0, 1, 2, 3, etc...) generate a color between
// gradStart and gradEnd - and makes it's best effort to generate colors that are as
// visually distinct from all the colors that it has already generated
// assuming that it is generating colors in ascending index order
// it generates colors based on a HSV color wheel, so they should look
// natural rather than a linear sliding of rgb values
// this function supports up to 256 colors before it starts repeating
...
// blendColors will blend between the hex colors defined in the given theme array
// depending on the position given (0.0 -> 1.0), 0.0 will always be the first color
// in the array. 1.0 will always be the final color in the array
...
function blendColors(theme: string[], pos: number): Color {
  ...
  // get the indexes of the two colors we are blending between
  const startIndex = Math.floor(maxIndex * pos) % theme.length;
  const endIndex = (startIndex + 1) % theme.length;

  // figure out where between these two colors we are
  ...
  // route round the color wheel, we want to detect 'long' routes and force them the
  // other way
  if (hueDiff > 180) {
